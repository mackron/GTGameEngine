...
namespace GT
{
    ...
    // immediately - this is because of efficiency.
    //
    ...
    public:
        ...
        /// Maps the given surface's pixel buffer so that data can be read and written to it directly.
        ///
        /// @param hSurface [in] The surface whose pixel buffer is being mapped.
        ///
        /// @return A pointer to the surface's pixel buffer.
        ///
        /// @remarks
        ///     This is a direct pointer to the raw data. There is no hand-holding here - you need to figure out synchronization yourself.
        ///     @par
        ///     If the surface has not yet had it's size set, the returned value will be null.
        ///     @par
        ///     The size of the buffer will be at least the width * height * 4 bytes. You cannot assume it will be any larger.
        ///     @par
        ///     You should unmap the pixel buffer with UnmapSurfacePixelBuffer().
        ///     @par
        ///     If the surface is resized, the returned pointer will become invalid. It is up to you to figure out how to work around that.
        void* MapSurfaceRenderBuffer(HGUISurface hSurface) const;

        /// Unmaps the given surface's pixel buffer.
        ///
        /// @param hSurface [in] The surface whose pixel buffer is being unmapped.
        /// @param data     [in] A pointer to the data that was returned by the corresponding call to MapSurfaceRenderBuffer(). This is only used for validation.
        void UnmapSurfaceRenderBuffer(HGUISurface hSurface, const void* data) const;
        ...
        /// Sets the surface the element is attached to.
        ///
        ...
        bool SetElementSurface(HGUIElement hElement, HGUISurface hSurface);

        /// Retrieves the surface of the given element.
        ///
        /// @param hElement [in] A handle to the element whose surface is being retrieved.
        ///
        /// @remarks
        ///     This function runs in linear time. This is not a simple getter method. If speed is important, the surface should
        ///     be managed manually by the application.
        HGUISurface FindElementSurface(HGUIElement hElement) const;
        ...
        ///
        /// @remarks
        ///     This should be paired with a called to BeginBatch(). This is where repainting will occur.
        void EndBatch();
