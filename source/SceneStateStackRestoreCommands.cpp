...
#include <GTGE/SceneStateStackRestoreCommands.hpp>
    ...
    void SceneStateStackRestoreCommands::Deserialize(Deserializer &deserializer)
    {
        ...
        deserializer.Read(header);
        {
            assert(header.id == Serialization::ChunkID_SceneStateStackRestoreCommands);
            {
                ...
                default:
                    {
                        g_EngineContext->Logf("Error deserializing SceneStateStackRestoreCommands. The main chunk is an unsupported version (%d).", header.version);
