...
namespace GT
{
    ...
    GraphicsAssetResource_Model* GraphicsAssetResourceManager::LoadModel(Asset* pAsset)
    {
        ...
        for (unsigned int iMesh = 0; iMesh < meshCount; ++iMesh)
        {
            ...
            for (unsigned int iMaterial = 0; iMaterial < materialCount; ++iMaterial)
            {
                ...
                char materialAbsolutePath[EASYVFS_MAX_PATH];
                if (easyvfs_find_absolute_path_explicit_base(m_assetLibrary.GetVFS(), materialName, basePath, materialAbsolutePath, sizeof(materialAbsolutePath)))
                {
            ...
            if (easymtl_initfromexisting_nocopy(&material, desc.pData, desc.dataSizeInBytes))
            {
                ...
                for (unsigned int iInput = 0; iInput < inputCount; ++iInput)
                {
                    ...
                    if (pInput != nullptr)
                    {
                        ...
                        if (pIdentifier != nullptr && pIdentifier->type == easymtl_type_tex2d)
                        {
                            ...
                            char textureAbsolutePath[EASYVFS_MAX_PATH];
                            if (easyvfs_find_absolute_path_explicit_base(m_assetLibrary.GetVFS(), pInput->path.value, basePath, textureAbsolutePath, sizeof(textureAbsolutePath)))
                            {
                ...
                for (unsigned int iProperty = 0; iProperty < propertyCount; ++iProperty)
                {
                    ...
                    if (pProperty != nullptr && pProperty->type == easymtl_type_tex2d)
                    {
                        ...
                        char textureAbsolutePath[EASYVFS_MAX_PATH];
                        if (easyvfs_find_absolute_path_explicit_base(m_assetLibrary.GetVFS(), pProperty->path.value, basePath, textureAbsolutePath, sizeof(textureAbsolutePath)))
                        {
                            pTextureResource = this->LoadTexture(textureAbsolutePath);
                        }
                        else
    ...
    {
        easyvfs_context* pVFS = m_assetLibrary.GetVFS();
        
        if (absolutePath != nullptr)
        {
            if (easyvfs_find_absolute_path(pVFS, assetPath, absolutePath, sizeof(absolutePath)))
            {
        ...
        {
            char tempAbsolutePath[EASYVFS_MAX_PATH];
            if (easyvfs_find_absolute_path(pVFS, assetPath, tempAbsolutePath, sizeof(tempAbsolutePath)))
            {
