...
#include <GTGE/Graphics/DefaultGraphicsWorld_OpenGL.hpp>
...
namespace GT
{
    ...
    struct RenderTarget_OpenGL
    {
        RenderTarget_OpenGL(uint32_t flagsIn)
            : flags(flagsIn),
              ...
              priority(0), projection(), view(), pGUIElement(0)
        {
        }
    ...
    struct MaterialResource_OpenGL : public Resource_OpenGL
    {
        ...
        struct InputVariables
        {
            ...
            GT::Vector<VariableDesc> variableDescriptors;
    ...
    struct MeshResource_OpenGL : public Resource_OpenGL
    {
        ...
        GT::Vector<MaterialSlot> materialSlots;
    };
    ...
    struct MeshObject_OpenGL : public Object_OpenGL, public SIMDAlignedType
    {
        ...
        GT::Vector<MaterialSlot> materials;
    };
            ...
            if (m_gl.MakeCurrent(m_gl.GetDummyDC(), m_hRC))
            {
                ...
                // GUI.
                {
                    GLuint guiRectangleVertexShader = this->CreateShader_GLSL(GL_VERTEX_SHADER,
                        ...
                        "uniform glm::vec4 Rect;\n"
                        "void main() {\n"
                        "    gl_Position = Projection * glm::vec4((VS_Position * Rect.zw) + Rect.xy, 0.0, 1.0);\n"
                        "}"
                    );
...

                {
                    GLuint guiTexturedRectangleVertexShader = this->CreateShader_GLSL(GL_VERTEX_SHADER,
                        ...
                        "uniform glm::mat4 Projection;\n"
                        "uniform glm::vec4 Rect;\n"
                        "uniform glm::vec4 UVOffsetAndScale;\n"
    ...
    HGraphicsResource DefaultGraphicsWorld_OpenGL::CreateMaterialResource(const GraphicsMaterialResourceDesc &materialDesc)
    {
        ...
        const char* vertexShaderSource =
            ...
            "    FS_Normal   = VS_Normal;\n"
            "    \n"
            "    gl_Position = Projection * View * Model * glm::vec4(VS_Position, 1.0);\n"
            "}";
    ...
    void DefaultGraphicsWorld_OpenGL::SetObjectTransform(HGraphicsObject hObject, const glm::vec4 &position, const glm::quat &rotation, const glm::vec4 &scale)
    {
    ...
    void DefaultGraphicsWorld_OpenGL::SetObjectPosition(HGraphicsObject hObject, const glm::vec4 &position)
    {
        auto pObject = reinterpret_cast<Object_OpenGL*>(hObject);
        if (pObject != nullptr)
        {
    ...
    void DefaultGraphicsWorld_OpenGL::SetObjectRotation(HGraphicsObject hObject, const glm::quat &rotation)
    {
        auto pObject = reinterpret_cast<Object_OpenGL*>(hObject);
        if (pObject != nullptr)
        {
    ...
    void DefaultGraphicsWorld_OpenGL::SetObjectScale(HGraphicsObject hObject, const glm::vec4 &scale)
    {
        auto pObject = reinterpret_cast<Object_OpenGL*>(hObject);
        if (pObject != nullptr)
        {
    ...
    HGraphicsObject DefaultGraphicsWorld_OpenGL::CreateMeshObject(HGraphicsResource hMeshResource, const glm::vec4 &position, const glm::quat &rotation, const glm::vec4 &scale)
    {
        auto pMeshResource = reinterpret_cast<MeshResource_OpenGL*>(hMeshResource);
        if (pMeshResource != nullptr)
        {
    ...
    void DefaultGraphicsWorld_OpenGL::SetRenderTargetProjectionAndView(HGraphicsRenderTarget hRT, const glm::mat4 &projection, const glm::mat4 &view)
    {
        auto pRT = reinterpret_cast<RenderTarget_OpenGL*>(hRT);
        if (pRT != nullptr)
        {
    ...
    void DefaultGraphicsWorld_OpenGL::SetRenderTargetProjection(HGraphicsRenderTarget hRT, const glm::mat4 &projection)
    {
        auto pRT = reinterpret_cast<RenderTarget_OpenGL*>(hRT);
        if (pRT != nullptr)
        {
    ...
    void DefaultGraphicsWorld_OpenGL::SetRenderTargetView(HGraphicsRenderTarget hRT, const glm::mat4 &view)
    {
        auto pRT = reinterpret_cast<RenderTarget_OpenGL*>(hRT);
        if (pRT != nullptr)
        {
    ...
    void DefaultGraphicsWorld_OpenGL::GetRenderTargetProjectionAndView(HGraphicsRenderTarget hRT, glm::mat4 &projectionOut, glm::mat4 &viewOut) const
    {
        auto pRT = reinterpret_cast<RenderTarget_OpenGL*>(hRT);
        if (pRT != nullptr)
        {
                    ...
                    assert(pMeshResource != nullptr);
                    {
                        ...
                        for (size_t iMaterial = 0; iMaterial < pMeshResource->materialSlots.GetCount(); ++iMaterial)
                        {
                            ...
                            if (pMaterialResource != nullptr)
                            {
                                ...
                                glm::mat4 model = Math::CalculateTransformMatrix(pMeshObject->position, pMeshObject->rotation, pMeshObject->scale);
