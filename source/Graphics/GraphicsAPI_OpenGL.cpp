...
#include <GTGE/Graphics/GraphicsAPI_OpenGL.hpp>
#include <GTGE/Core/Strings/Tokenizer.hpp>      // For extracting extensions.
    ...
    bool GraphicsAPI_OpenGL::IsExtensionSupported(const char* extension) const
    {
        if (this->GetStringi != nullptr)
        {
            ...
            for (GLuint iExtension = 0; iExtension < extensionCount; ++iExtension)
            {
                ...
                if (GT::Strings::Equal<false>(extensionSrc, extension))
                {
                    return true;
                }
            }
        ...
        else
        {
            ...
            {
                GT::Strings::WhitespaceTokenizerUTF8 iExtension(extensions);
                while (iExtension)
                {
                    if (GT::Strings::Equal<false>(iExtension.start, iExtension.GetSizeInTs(), extension))
                    {
                        return true;
                    }
        ...
        {
            GT::Strings::WhitespaceTokenizerUTF8 iExtension(extensionsWGL);
            while (iExtension)
            {
                if (GT::Strings::Equal<false>(iExtension.start, iExtension.GetSizeInTs(), extension))
                {
