...
namespace GT
{
    ...
    struct OpenALListener : public SIMDAlignedType
    {
    ...
    HPlaybackDevice AudioEngine_OpenAL::OpenPlaybackDevice(size_t hDeviceIndex)
    {
        ...
        if (handle == 0)
        {
            // Failed to create hDevice handle.
    ...
    void AudioEngine_OpenAL::SetListenerPosition(HListener hListener, vec3 position)
    {
        auto pListener = this->GetListenerPtr(hListener);
        if (pListener != nullptr)
        {
    ...
    vec3 AudioEngine_OpenAL::GetListenerPosition(HListener hListener) const
    {
        auto pListener = this->GetListenerPtr(hListener);
        if (pListener != nullptr)
        {
    ...
    void AudioEngine_OpenAL::SetListenerOrientation(HListener hListener, const quat &orientation)
    {
        auto pListener = this->GetListenerPtr(hListener);
        if (pListener != nullptr)
        {
    ...
    quat AudioEngine_OpenAL::GetListenerOrientation(HListener hListener) const
    {
        auto pListener = this->GetListenerPtr(hListener);
        if (pListener != nullptr)
        {
    ...
    void AudioEngine_OpenAL::SetSoundPosition(HSound hSound, vec3 position)
    {
        auto pSound = this->GetSoundPtr(hSound);
        if (pSound != nullptr)
        {
    ...
    vec3 AudioEngine_OpenAL::GetSoundPosition(HSound hSound) const
    {
        auto pSound = this->GetSoundPtr(hSound);
        if (pSound != nullptr)
        {
