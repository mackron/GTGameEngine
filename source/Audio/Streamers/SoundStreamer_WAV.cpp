...
namespace GT
{
    ...
    bool SoundStreamer_WAV::Initialize(const char* filePath, FileSystem &fileSystem)
    {
        // TODO: Improve this so that the file handle is stored and we just read data from it chunks at a time in ReadNextChunk().
        //
        ...
        if (hFile != NULL)
        {
            ...
            if (m_fileDataSizeInBytes > 0)
            {
                ...
                result = this->ReadRIFFHeaderAndChunks();
            }
    ...
    void SoundStreamer_WAV::Uninitialize()
    {
        free(m_fileDataPtr);
    ...
    bool SoundStreamer_WAV::ReadRIFFHeaderAndChunks()
    {
	    ...
	    {
            // FMT and/or DATA chunks were not found.
		    return false;
	    }
