...
namespace GTEngine
{
    ...
    bool SoundStreamer_WAV::Open()
    {
        ...
        {
            this->file = GTCore::IO::Open(this->absolutePath.c_str(), GTCore::IO::OpenMode::Read);
            if (this->file != nullptr)
            {
    ...
    uint32_t SoundStreamer_WAV::ReadChunk(void* data)
    {
        ...
        if (static_cast<uint64_t>(GTCore::IO::Tell(this->file)) == this->dataSize + this->dataStartPos)
        {
        ...
        GTCore::IO::Read(this->file, data, bytes_to_read);
    ...
    bool SoundStreamer_WAV::ReadRIFFHeader()
    {
	    ...
	    char riff[4];
        GTCore::IO::Read(this->file, riff, 4);
	    ...
	    uint32_t fileSize;
        GTCore::IO::Read(this->file, &fileSize, 4);
	    if (fileSize == 0)
	    {
	    ...
	    // file is a WAVE file.
	    char wave[4];
        GTCore::IO::Read(this->file, wave, 4);
    ...
    bool SoundStreamer_WAV::ReadRIFFChunks()
    {
        ...
        while ((!found_fmt || !found_data) && !GTCore::IO::AtEnd(this->file))
	    {
		    // Read in the name and size of the chunk.
            GTCore::IO::Read(this->file, chunk_name, 4);
            GTCore::IO::Read(this->file, &chunk_size, 4);

		    // Now check the name of the chunk and perform the appropriate operations.
            if (GTCore::Strings::Equal<false>(chunk_name, "fmt "))
		    {
                ...
                GTCore::IO::Read(this->file, &compression_code, 2);
                ...
                GTCore::IO::Read(this->file, &this->numChannels, 2);
            ...
            else if (GTCore::Strings::Equal<false>(chunk_name, "data"))
		    {
                ...
                this->dataStartPos = static_cast<uint64_t>(GTCore::IO::Tell(this->file));
