...
#include <GTGameEngine/Audio/SoundPlaybackThreadProc.hpp>
...
namespace GT
{
    ...
    void SoundPlaybackThreadProc::Initialize(HSound source, HAudioBuffer buffers[2], SoundStreamer &streamer)
    {
        assert(m_source == 0);
            ...
            if (processedBufferCount > 0)
            {
                if (m_playbackState != SoundPlaybackState_Stopped)      // <-- The sound playback may have stopped while waiting for data.
                {
                    if (processedBufferCount == 1)
                    {
                        ...
                        HAudioBuffer hTempBuffer = m_buffers[0];
    ...
    bool SoundPlaybackThreadProc::ReadAndQueueNextChunk(HAudioBuffer buffer)
    {
        assert(buffer     != 0);
        assert(m_source   != 0);
        assert(m_streamer != nullptr);
