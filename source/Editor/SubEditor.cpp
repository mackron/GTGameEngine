...
namespace GT
{
          ...
          m_isMarkedAsModified(false), m_isMarkingAsModifiedEnabled(true)
    {
        ...
        assert(script.IsTable(-1));
        {
            ...
            {
                script.PushValue(-2);
                script.Push(drpath_file_name(m_absolutePath.c_str()));
                script.Call(2, 1);
    ...
    void SubEditor::MarkAsModified()
    {
        if (m_isMarkingAsModifiedEnabled)
        {
            if (!m_isMarkedAsModified)
            {
                ...
                String tabText(drpath_file_name(this->GetRelativePath())); tabText += "*";
    ...
    void SubEditor::UnmarkAsModified()
    {
        if (m_isMarkedAsModified)
        {
            ...
            this->SetTabText(drpath_file_name(this->GetRelativePath()));
