...
namespace GTEngine
{
    ...
    void RCSetTextureState::SetTexture2DData(TextureState_OpenGL21* textureStateIn, GLenum targetIn, int mipmap, GTImage::ImageFormat format, unsigned int width, unsigned int height, const void* data, size_t dataSizeInBytes)
    {
        assert((this->textureState == nullptr || (this->textureState == textureStateIn && this->target == targetIn)) && textureStateIn != nullptr);
        {
            TextureData mipmapData;
