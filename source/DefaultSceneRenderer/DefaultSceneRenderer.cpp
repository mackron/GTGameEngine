...
namespace GT
{
    ...
    {
        // We create a framebuffer for every viewport. We map these to viewports with a simple Map container.
    ...
    {
        if (Strings::Equal(name, "HDRExposure"))
        {
            this->SetHDRExposure(value);
        }
        else if (Strings::Equal(name, "BloomFactor"))
        {
    ...
    void DefaultSceneRenderer::SetProperty(const char* name, bool value)
    {
        ...
        else if (Strings::Equal(name, "IsHDREnabled"))
        {
            if (value)
            {
                this->EnableHDR();
        ...
        else if (Strings::Equal(name, "IsBloomEnabled"))
        {
            if (value)
            {
                this->EnableBloom();
    ...
    {
        if (Strings::Equal(name, "BackgroundClearColour"))
        {
    ...
    String DefaultSceneRenderer::GetStringProperty(const char* name) const
    {
        (void)name;
    ...
    void DefaultSceneRenderer::RenderPointShapowMapFace(const DefaultSceneRendererPointLight &light, const glm::mat4 &faceViewMatrix, int faceIndex, const Vector<DefaultSceneRendererMesh> &meshes)
    {
