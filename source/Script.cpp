...
#pragma warning(pop)
    ...
    bool Script::LoadFile(drfs_context* pVFS, const char* fileName)
    {
        // We don't use Lua's built in file loading API. Instead we use our own virtual file system.

        char* pFileData = drfs_open_and_read_text_file(pVFS, fileName, nullptr);
    ...
    bool Script::LoadFileAndExecute(drfs_context* pVFS, const char* filename, int returnValueCount)
    {
