...
namespace GT
{
    ...
    struct ScriptRayTestCallback : public GTEngine::RayTestCallback
    {
    public:
        ...
        bool NeedsCollision(short collisionGroupIn, short collisionMaskIn, GTEngine::SceneNode &otherSceneNode)
        {
            ...
            if (result)
            {
                ...
                assert(m_script.IsTable(-1));
                {
                    ...
                    if (m_script.IsFunction(-1))
                    {
                        ...
                        PushSceneNode(m_script, otherSceneNode);     // otherSceneNode
                        m_script.Call(2, 1);
                        {
        ...
        void ProcessResult(GTEngine::SceneNode &sceneNode, const glm::vec3 &worldPosition, const glm::vec3 &worldNormal)
        {
            assert(m_script.IsTable(-1));
            {
                ...
                {
                    m_script.PushValue(-2);                                 // self
                    PushSceneNode(m_script, sceneNode);          // sceneNode
                    PushNewVector3(m_script, worldPosition);     // worldPosition
                    PushNewVector3(m_script, worldNormal);       // worldNormal
                    m_script.Call(4, 0);
                }
                else
                {
    ...
    namespace SceneFFI
    {
        ...
        int CreateNewSceneNode(GTLib::Script &script)
        {
            ...
            if (scene != nullptr)
            {
                ...
                {
                    auto prefab = GTEngine::PrefabLibrary::Acquire(script.ToString(2));
                    if (prefab != nullptr)
                    {
            ...
            if (scene != nullptr)
            {
                ...
                if (static_cast<size_t>(viewportIndex) < scene->GetViewportCount())
                {
                    auto &viewport = scene->GetViewportByIndex(viewportIndex);
                    {
                        ...
                        if (sceneNode != nullptr)
                        {
                            auto cameraComponent = sceneNode->GetComponent<GTEngine::CameraComponent>();
        ...
        int CalculateViewportPickingRay(GTLib::Script &script)
        {
            ...
            if (scene != nullptr)
            {
                ...
                PushNewVector3(script, rayStart);
                PushNewVector3(script, rayEnd);
        ...
        int RayTest(GTLib::Script &script)
        {
            ...
            if (scene != nullptr)
            {
                glm::vec3 rayNear = ToVector3(script, 2);
                glm::vec3 rayFar  = ToVector3(script, 3);
                ...
                if (script.IsTable(4))
                {
                    ...
                    ScriptRayTestCallback callback(script);
                        
                    auto result = scene->RayTest(rayNear, rayFar, callback);
                    if (result != nullptr)
                    {
                        PushSceneNode(script, *result);
                    }
                    else
                    {
                        script.PushNil();
        ...
        int SetGravity(GTLib::Script &script)
        {
            ...
            if (scene != nullptr)
            {
                scene->SetGravity(ToVector3(script, 2));
            }
        ...
        int GetGravity(GTLib::Script &script)
        {
            ...
            if (scene != nullptr)
            {
                PushNewVector3(script, scene->GetGravity());
            }
            else
            {
                PushNewVector3(script, 0.0f, -9.81f, 0.0f);
            }
